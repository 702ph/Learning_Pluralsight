...
def read_and_preprocess(image_bytes, label = None, augment = False):
    ...
    image = tf.image.decode_jpeg(image_bytes, NUM_CHANNELS)
    ...
    if augment:
        ...
        # この２つはいずれにしても必要なようだ。
        # ただし、cropする際はサイズを拡張しておく必要がある
        image = tf.image.resize_bilinear(images = image, size = [HEIGHT, WIDTH], align_corners = False)
        image = tf.squeeze(input = image, axis = 0) # remove batch dimension #なぜなくすのだ？     
        
        # ビデオでは、crop, flip, contrastもaugmentationしていたが、ひとまず自分としてはbrightnessだけで試してみることにする。
