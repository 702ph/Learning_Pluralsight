...
def dnn_dropout_model(img, mode, hparams):
    ...
    dropb_probability = hparams.get("dprob", 0.1) #if there is key "dropb", use value of the key. otherwise, usese 0.1

    X = tf.reshape(tensor = img, shape = [-1, HEIGHT * WIDTH]) #flatten
    a1 = tf.layers.dense(inputs = X, units = 300, activation = tf.nn.relu)
    a2 = tf.layers.dense(inputs = a1, units = 100, activation = tf.nn.relu)
    a3 = tf.layers.dense(inputs = a2, units = 30, activation = tf.nn.relu)

    #training = true　のときのみ、dropoutする。evaluationの時はしない。
    dropout_layer = tf.layers.dropout(inputs=a3, rate=dropb_probability, training=training_mode)

    ylogits = tf.layers.dense(inputs = dropout_layer, units = NCLASSES, activation = None)
    return ylogits, NCLASSES
